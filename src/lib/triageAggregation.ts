...
import { runsForPlatform, stepAppliesToPlatform } from './platforms';
...
/** Same shape as aggregateProject but scoped to a single platform.
 *  Filters runs to that platform AND filters steps to those that apply
 *  to that platform. Used by the split triage view and (in Phase 2) by
 *  the comparison view. */
export function aggregateProjectByPlatform(
  allRuns: TestRunData[],
  platform: string
): ProjectAggregate {
  const platformRuns = runsForPlatform(allRuns, platform);

  if (platformRuns.length === 0) {
    return {
      projectName: allRuns[0]?.projectName || '',
      testCycle: allRuns[0]?.testCycle,
      platform,
      runs: [],
      testers: [],
      steps: [],
  ...
  // Filter the steps array of each run to only platform-applicable steps.
  // We don't mutate the original runs — we make scoped copies.
  const scopedRuns = platformRuns.map(run => ({
    ...run,
    steps: (run.steps || []).filter(s => stepAppliesToPlatform(s, platform)),
  }));

  // Reuse existing aggregateProject logic on the scoped runs.
  const aggregate = aggregateProject(scopedRuns);

  // Tag the aggregate with the platform so consumers know what scope they have.
