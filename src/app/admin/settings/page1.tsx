...
export default function WorkspaceSettings() {
  ...
  // Compute what has changed for the Save Bar
  const changedSections = [];
  if (formData.workspaceName !== initialData.workspaceName || formData.timezone !== initialData.timezone || formData.supportEmail !== initialData.supportEmail) changedSections.push('General');
  if (formData.jiraUrl !== initialData.jiraUrl || formData.jiraEmail !== initialData.jiraEmail || formData.jiraToken !== initialData.jiraToken) changedSections.push('Integrations');
  if (formData.aiProvider !== initialData.aiProvider || formData.aiToken !== initialData.aiToken || formData.maxSpend !== initialData.maxSpend) changedSections.push('AI Configuration');
  if (formData.notifyDigest !== initialData.notifyDigest || formData.notifyAlerts !== initialData.notifyAlerts || formData.notifyComplete !== initialData.notifyComplete || formData.notifyStuck !== initialData.notifyStuck || formData.notifyWeekly !== initialData.notifyWeekly) changedSections.push('Notifications');
  ...
  // --- SCROLL SPY & SHORTCUTS ---
  useEffect(() => {
    const handleScroll = () => {
      const scrollPos = window.scrollY + 140;
      const sections = ['general', 'integrations', 'ai', 'team', 'notifications', 'webhooks', 'branding', 'audit', 'danger'];
    ...
    <div className="app-container">
      <style dangerouslySetInnerHTML={{__html: `
        ...
        :root, .app-container {
          ...
          --rose: #a6421f; --rose-soft: rgba(166,66,31,0.1);
          --radius: 6px;
        }

        .app-container * { box-sizing: border-box; }
        .app-container { min-height: 100vh; background: var(--bg); font-family: 'IBM Plex Sans', system-ui, sans-serif; color: var(--ink); font-size: 14px; -webkit-font-smoothing: antialiased; }
        ...
        .section-nav-item svg { flex-shrink: 0; opacity: 0.6; }
        .section-nav-item.active svg { opacity: 1; }
        .section-nav-item .warning-dot { margin-left: auto; width: 6px; height: 6px; border-radius: 50%; background: var(--warn); }
        ...
        .page-eyebrow { font-family: 'JetBrains Mono', monospace; font-size: 10px; text-transform: uppercase; letter-spacing: 0.14em; color: var(--ink-mute); margin-bottom: 6px; display: flex; align-items: center; gap: 6px; }
        .page-eyebrow .sep { opacity: 0.4; }
        .page-title { font-family: 'Fraunces', serif; font-size: 36px; font-weight: 600; letter-spacing: -0.025em; margin: 0 0 6px; line-height: 1.05; }
        .page-title em { font-style: italic; color: var(--accent); font-weight: 500; }
        .page-sub { color: var(--ink-mute); font-size: 14px; margin: 0; max-width: 560px; line-height: 1.5; }
        ...
        .section-title { font-family: 'Fraunces', serif; font-size: 22px; font-weight: 600; letter-spacing: -0.01em; margin: 0; display: flex; align-items: center; gap: 10px; }
        ...
        .field-label-name .req { font-family: 'JetBrains Mono', monospace; font-size: 9px; color: var(--rose); text-transform: uppercase; letter-spacing: 0.08em; }
        ...
        .input-action-btn { position: absolute; right: 6px; width: 28px; height: 28px; border: none; background: transparent; border-radius: 4px; color: var(--ink-mute); cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.15s ease; }
        .input-action-btn:hover { background: var(--surface-alt); color: var(--ink); }

        .input-hint { font-size: 11px; color: var(--ink-mute); font-family: 'JetBrains Mono', monospace; display: flex; align-items: center; gap: 6px; margin-top: 6px; }
        ...
        .integration-logo.slack { background: linear-gradient(135deg, #4a154b 0%, #e01e5a 100%); color: #fff; }
        .integration-logo.figma { background: linear-gradient(135deg, #f24e1e 0%, #a259ff 100%); }
        .integration-logo.github { background: #24292e; }
        ...
        .conn-status.error { background: var(--fail-soft); color: var(--fail); }
        .conn-status.error::before { content: ''; width: 6px; height: 6px; border-radius: 50%; background: var(--fail); }
        ...
        .icon-ghost.danger:hover { color: var(--fail); background: var(--fail-soft); }

        /* WEBHOOKS */
        .webhook-row { display: grid; grid-template-columns: 1fr 120px 80px auto; gap: 16px; align-items: center; padding: 14px 24px; border-bottom: 1px solid var(--line); }
        ...
        .logo-upload-text .upload-main { font-size: 13px; font-weight: 500; color: var(--ink); margin-bottom: 2px; }
        .logo-upload-text .upload-sub { font-size: 11px; color: var(--ink-mute); font-family: 'JetBrains Mono', monospace; }
        .color-swatches { display: grid; grid-template-columns: repeat(6, 1fr); gap: 6px; margin-top: 10px; }
        .color-swatch { aspect-ratio: 1; border-radius: 6px; cursor: pointer; border: 2px solid transparent; transition: all 0.15s ease; position: relative; }
        ...
        .brand-preview-email-logo { width: 20px; height: 20px; background: var(--accent); border-radius: 4px; display: flex; align-items: center; justify-content: center; color: #fff; }
        ...
        /* AUDIT LOG */
        .audit-row { display: grid; grid-template-columns: auto 1fr auto; gap: 14px; align-items: center; padding: 14px 24px; border-bottom: 1px solid var(--line); }
        ...
        .audit-text b { color: var(--ink); font-weight: 500; }
        .audit-text .action-name { font-family: 'JetBrains Mono', monospace; font-size: 11px; padding: 1px 6px; background: var(--surface-alt); border: 1px solid var(--line); border-radius: 3px; color: var(--ink-soft); }
        ...
        .save-bar .btn-discard { height: 36px; background: transparent; color: rgba(255,255,255,0.7); border: 1px solid rgba(255,255,255,0.15); padding: 0 14px; border-radius: 6px; font-family: inherit; font-size: 13px; font-weight: 500; cursor: pointer; }
        .save-bar .btn-discard:hover { color: #fff; background: rgba(255,255,255,0.08); }
        .save-bar .btn-save { height: 36px; background: var(--accent); color: #fff; border: none; padding: 0 16px; border-radius: 6px; font-family: inherit; font-size: 13px; font-weight: 500; cursor: pointer; display: inline-flex; align-items: center; gap: 6px; }
        .save-bar .btn-save:hover { background: #3a5d4f; }
        .save-bar .btn-save kbd { font-family: 'JetBrains Mono', monospace; font-size: 10px; padding: 1px 5px; background: rgba(255,255,255,0.15); border-radius: 3px; color: rgba(255,255,255,0.8); margin-left: 4px; }
        ...
        @media (max-width: 1100px) {
          .main { grid-template-columns: 1fr; } .section-nav { display: none; }
          .content { padding: 32px 24px 120px; }
          .field-row { grid-template-columns: 1fr; gap: 12px; }
          .brand-layout { grid-template-columns: 1fr; } .brand-preview-card { position: static; }
          .webhook-row { grid-template-columns: 1fr auto; } .webhook-events { display: none; }
        }
        ...
        <main className="main">
          ...
          <aside className="section-nav">
            ...
            <nav className="section-nav-list">
              <a className={`section-nav-item ${activeSection === 'general' ? 'active' : ''}`} onClick={(e) => scrollToSection(e, 'general')}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="3"/><path d="M12 1v6m0 10v6M4.22 4.22l4.24 4.24m7.08 7.08l4.24 4.24M1 12h6m10 0h6M4.22 19.78l4.24-4.24m7.08-7.08l4.24-4.24"/></svg> General
              </a>
              <a className={`section-nav-item ${activeSection === 'integrations' ? 'active' : ''}`} onClick={(e) => scrollToSection(e, 'integrations')}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M10 13a5 5 0 007 0l4-4a5 5 0 00-7-7l-1 1"/><path d="M14 11a5 5 0 00-7 0l-4 4a5 5 0 007 7l1-1"/></svg> Integrations
                <span className="warning-dot"></span>
              </a>
              <a className={`section-nav-item ${activeSection === 'ai' ? 'active' : ''}`} onClick={(e) => scrollToSection(e, 'ai')}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg> AI Configuration
              </a>
              <a className={`section-nav-item ${activeSection === 'team' ? 'active' : ''}`} onClick={(e) => scrollToSection(e, 'team')}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg> Team &amp; Permissions
              </a>
              <a className={`section-nav-item ${activeSection === 'notifications' ? 'active' : ''}`} onClick={(e) => scrollToSection(e, 'notifications')}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0"/></svg> Notifications
              </a>
              <a className={`section-nav-item ${activeSection === 'webhooks' ? 'active' : ''}`} onClick={(e) => scrollToSection(e, 'webhooks')}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg> Webhooks
              </a>
              <a className={`section-nav-item ${activeSection === 'branding' ? 'active' : ''}`} onClick={(e) => scrollToSection(e, 'branding')}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="13.5" cy="6.5" r=".5"/><circle cx="17.5" cy="10.5" r=".5"/><circle cx="8.5" cy="7.5" r=".5"/><circle cx="6.5" cy="12.5" r=".5"/><path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10c.83 0 1.5-.67 1.5-1.5 0-.39-.15-.74-.39-1.01-.23-.26-.38-.61-.38-.99 0-.83.67-1.5 1.5-1.5H16c3.31 0 6-2.69 6-6 0-4.96-4.5-9-10-9z"/></svg> Branding
              </a>
              ...
              <a className={`section-nav-item ${activeSection === 'audit' ? 'active' : ''}`} onClick={(e) => scrollToSection(e, 'audit')}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg> Audit Log
              </a>
              <a className={`section-nav-item ${activeSection === 'danger' ? 'active' : ''}`} onClick={(e) => scrollToSection(e, 'danger')} style={{color: 'var(--fail)'}}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> Danger Zone
              </a>
            </nav>
          </aside>
          ...
          <div className="content">
            <div className="page-head">
              <div>
                <div className="page-eyebrow"><span>Workspace</span><span className="sep">/</span><span>Settings</span></div>
                <h1 className="page-title">Workspace <em>settings</em></h1>
                <p className="page-sub">Configure how your agency works — integrations, AI, team, notifications, and more.</p>
              </div>
            </div>
            ...
            <section className="section" id="integrations">
              ...
              <div className="section-card">
                <div className="integration-head no-border-bottom">
                  ...
                  <div className="integration-meta">
                    <h3>Figma</h3>
                    <p>Embed design references directly inside test scripts.</p>
                  </div>
                  <span className="conn-status disconnected">Not connected</span>
                </div>
                <div className="integration-foot">
                  <span className="integration-foot-meta">Requires Figma Professional or Enterprise</span>
            ...
            <section className="section" id="ai">
              <div className="section-head">
                ...
                <p className="section-description">The LLM used to generate Jira ticket summaries from tester notes.</p>
              </div>
              <div className="section-card">
                <div className="field-row">
                  <div className="field-label">
                ...
                <div className="field-row">
                  ...
                  <div className="field-input">
                    <div className="input-with-action">
                      <input className="input" type={showAiToken ? "text" : "password"} placeholder="sk-••••••••••••••••••••••••" value={formData.aiToken} onChange={e => setFormData({...formData, aiToken: e.target.value})} />
            ...
            <section className="section" id="danger">
              <div className="section-head">
                <h2 className="section-title" style={{color: 'var(--fail)'}}>Danger zone</h2>
              ...
              <div className="danger-card">
                ...
                <div className="field-row">
                  <div className="field-label">
                    <div className="field-label-name" style={{color: 'var(--ink)'}}>Delete workspace</div>
                    <div className="field-label-desc">Permanently deletes all projects, testers, scripts, and integrations.</div>
                  </div>
                  <div className="field-input" style={{flexDirection: 'row', alignItems: 'center'}}>
                    <button className="btn btn-danger" style={{background: 'var(--fail)', color: '#fff', borderColor: 'var(--fail)'}}>
